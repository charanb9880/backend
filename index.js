...
/* ======================
   CORS FIX — FULL MULTI-ORIGIN SUPPORT
====================== */
...
app.post('/api/admin/assign-cash/:userId', verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (!Number.isFinite(am)) return res.status(400).json({ error: "Invalid amount" });
...
/* ======================
   NEW: BULK APPROVE ALL + ASSIGN FUNDS
====================== */
...
app.post('/api/admin/set-virtual-cash', verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (!Number.isFinite(vc)) return res.status(400).json({ error: "Invalid virtual_cash" });
