...
app.post("/api/trade/buy", verifyToken, async (req, res) => {
  ...
  if (!pos.rowCount) {
    await pool.query(`INSERT INTO portfolio(user_id,symbol,quantity,average_price) VALUES ($1,$2,$3,$4)`, [uid, symbol, qty, price]);
  } else {
    ...
    await pool.query(`UPDATE portfolio SET quantity=$1,average_price=$2 WHERE user_id=$3 AND symbol=$4`, [qNew, avg, uid, symbol]);
  }
...
app.post("/api/trade/sell", verifyToken, async (req, res) => {
  ...
  if (!pos.rowCount || qty > pos.rows[0].quantity) return res.status(400).json({ error: "Not enough shares" });
  ...
  if (qty === pos.rows[0].quantity)
    ...
    await pool.query(`UPDATE portfolio SET quantity=$1 WHERE user_id=$2 AND symbol=$3`, [pos.rows[0].quantity - qty, uid, symbol]);
...
app.listen(PORT, () => {
  ...
  runPriceFetcher();
  setInterval(runPriceFetcher, 10000);
});
